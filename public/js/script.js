...
function switchTab(tab) {
    // Get all panels - use optional chaining for null safety
    const panels = {
    ...
    // Tab order: 0=calc, 1=conv, 2=currency, 3=finance, 4=health
    const tabIndex = {
        'calc': 0,
        'conv': 1,
        'currency': 2,
...
function saveHistory(operationText) {
    ...
    fetch('/save-history', {
        ...
        body: JSON.stringify({ operasi: operationText })
    })
...
document.addEventListener('DOMContentLoaded', function () {
    let savedTheme = localStorage.getItem('econcalc-theme');

    // Prefer server settings if available
    if (window.userSettings && window.userSettings.theme) {
        savedTheme = window.userSettings.theme;
        // Update local storage to match server
        localStorage.setItem('econcalc-theme', savedTheme);
    }

    if (!savedTheme) savedTheme = 'dark'; // Default
